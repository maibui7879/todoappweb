...
// import { useNavigate } from "react-router-dom"; // KHÔNG CẦN DÙNG NỮA
import dayjs from "dayjs";
import relativeTime from "dayjs/plugin/relativeTime";
import "dayjs/locale/vi";

// Import TaskInfoModal (Bạn chỉnh lại đường dẫn cho đúng với cấu trúc thư mục nhé)
...
const NotificationPopover = () => {
  ...
  // [THÊM MỚI] State để quản lý việc mở Modal xem chi tiết Task
  const [selectedTaskId, setSelectedTaskId] = useState<string | null>(null);

  const { notifications, unreadCount, loadHistory ,markAllAsRead } = useNotification(); 
  // const navigate = useNavigate(); // XÓA BỎ
  ...
  // [SỬA ĐỔI] Xử lý khi click vào thông báo
  const handleNotificationClick = async (noti: any) => {
    try {
      ...
      // Thay vì chuyển hướng trang, ta lưu taskId để mở Modal
      if (noti.taskId) {
        setSelectedTaskId(noti.taskId);
      }
    } catch (error) {
      console.error(error);
    }
  ...
  return (
    <>
      ...
      {/* [THÊM MỚI] Render Modal hiển thị chi tiết Task */}
      {selectedTaskId && (
        <TaskInfoModal 
